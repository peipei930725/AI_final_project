...
def display_results(results, documents):
    ...
    for idx, (full_doc_id, score) in enumerate(results, start=1):  # 遍歷搜尋結果
        stock_code, date, doc_id = full_doc_id.split('-')  # 拆分文件 ID 獲取股票代碼、日期和序號
        print(f"{idx}. [股票代碼: {stock_code}] [日期: {date}] {doc_map[full_doc_id].strip()} (分數: {score:.2f})")  # 輸出結果

# === 步驟 6: 主程式 ===
if __name__ == "__main__":
    # 從資料夾中載入數據集
    data_directory = "data"  # 資料目錄路徑
    ...
    while True:
        ...
        if results:  # 如果有結果
            print("\n搜尋結果：")
            display_results(results, documents)  # 顯示結果
        else:
